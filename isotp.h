...
typedef struct IsoTpLink {
    ...
    /* message buffer */
    uint_least8_t*                    send_buffer;
    uint16_t                    send_buf_size;
    uint16_t                    send_size;
    uint16_t                    send_offset;
    /* multi-frame flags */
    uint_least8_t                     send_sn;
    uint16_t                    send_bs_remain; /* Remaining block size */
    uint_least8_t                     send_st_min;    /* Separation Time between consecutive frames, unit millis */
    uint_least8_t                     send_wtf_count; /* Maximum number of FC.Wait frame transmissions  */
...
void isotp_init_link(IsoTpLink *link, uint32_t sendid, 
                     uint_least8_t *sendbuf, uint16_t sendbufsize,
                     uint_least8_t *recvbuf, uint16_t recvbufsize);
